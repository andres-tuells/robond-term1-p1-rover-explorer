...
#values used to detect obstacles
def obstacle_thresh(img):
    return color_thresh(img, rgb_thresh=(0, 0, 0), rgb_thresh_max=(160, 160, 160))
...
def sample_thresh(img):
    return color_thresh(img, rgb_thresh=(100, 100, 0), rgb_thresh_max=(210,210,55))
...
def perspect_transform(img, src, dst):       
    ...
    return cv2.warpPerspective(img, M, (img.shape[1], img.shape[0]))# keep same size as input image
...
def perception_step(Rover):
    ...
    navigable = terrain_thresh(warped)
    ...
    world_size=200
    scale=10
    obstacles_x_world, obstacles_y_world = pix_to_world(obstacles_x, obstacles_y, x, y, yaw, world_size, scale)
    rocks_x_world, rocks_y_world         = pix_to_world(rocks_x, rocks_y, x, y, yaw, world_size, scale)
    navigable_x_world, navigable_y_world = pix_to_world(navigable_x, navigable_y, x, y, yaw, world_size, scale)
